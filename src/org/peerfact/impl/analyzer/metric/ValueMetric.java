...
/**
 * A metric which provides one value.
 * 
 ...
 *            the value type
 */
public class ValueMetric<Peer, Num extends Number> implements Metric<Peer, Num> {

	private String name;

	private String unit;

	private Map<Peer, Num> peerValues = new LinkedHashMap<Peer, Num>();

	private Num currentValue;

	public ValueMetric(String name, String unit) {
		this.name = name;
		this.unit = unit;
	}
	...
	public void generateTimePlots(Writer script, String analyzer, int startIndex)
		...
		script.write("plot '" + analyzer + AbstractFileAnalyzer.FILE_EXTENSION
				+ "' using 1:" + startIndex + " title 'Value of " + getName()
	...
	public List<String> getTimeMeasurementValues(long currentTime) {
		return Collections.singletonList(currentValue.toString());
	}
	...
	public void updateValue(Peer peer, Num value) {
		if (!peerValues.containsKey(peer)) {
			peerValues.put(peer, value);
		} else {
			peerValues.put(peer, value);
		}
		currentValue = value;
