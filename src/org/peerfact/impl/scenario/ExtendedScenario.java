...
class ExtendedScenario implements Scenario, Configurable {
	...
	private List<Class<? extends Component>> registeredComponentClasses;

	public ExtendedScenario(Class<? extends Component> defaultCompClass,
			List<Class<? extends Component>> additionalCompClasses,
		...
		registeredComponentClasses = new LinkedList<Class<? extends Component>>();
	...
	void createAction(Host host, Class<? extends Component> componentClass1,
			long time,
			String methodName, String[] paramStrings) {
		Component target = host.<Component> getComponent(componentClass1);
		if (target == null) {
			throw new IllegalStateException("Cannot find target implementing "
					+ componentClass1 + " in host " + host);
		}
	 ...
	 * 
	 * @see
	 * org.peerfact.api.scenario.Scenario#createActions(java.lang.String,
	...
	public int createActions(String hostIds, String timeInterval,
		...
		// select component
		Class<? extends Component> selectedClass = null;
		...
		if (methodName.contains(":")) {
			...
			for (Class<? extends Component> type : registeredComponentClasses) {
		...
		for (int i = 0; i < hosts.size(); i++) {
			createAction(hosts.get(i), selectedClass, time[i], method,
					params);
	 ...
	 *         parameter
	 */
	static long[] createTimePoints(int size, String boundsAsString) {
	 ...
	 *         and upper interval.
	 */
	private static long[] linearTimePoints(int size, long lower, long upper) {
		long[] times = new long[size];
	 ...
	 *         and upper interval.
	 */
	private static long[] exponentialTimePoints(int size, long lower, long upper) {
		long[] times = new long[size];
		if (size == 1) {// special case: a group contains only one host, then
			// the time point is the middle of the interval!
			times[0] = (upper + lower) / 2;
	 ...
	 */
	private static long exp(int y, int maxY, long maxTime) {
