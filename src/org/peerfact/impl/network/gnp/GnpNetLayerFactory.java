...
public class GnpNetLayerFactory extends AbstractNetLayerFactory {
	...
	private LinkedHashMap<IPv4NetID, GnpHostInfo> hostPool;

	private LinkedHashMap<String, ArrayList<IPv4NetID>> namedGroups;
	...
	public void setGnpFile(String gnpFileName) {
		...
		hostPool = new LinkedHashMap<IPv4NetID, GnpHostInfo>();
		namedGroups = new LinkedHashMap<String, ArrayList<IPv4NetID>>();
		...
		for (Object obj : root.elements()) {
			...
			if (elem.getName().equals("GroupLookup")) {
				for (Iterator<Element> iter = elem.elementIterator("Group"); iter
						.hasNext();) {
					Element variable = iter.next();
					String id = variable.attributeValue("id");

					ArrayList<IPv4NetID> group = new ArrayList<IPv4NetID>();
					for (Iterator<Element> ipIter = variable
							.elementIterator("IPs"); ipIter
							.hasNext();) {
						Element ipElement = ipIter.next();
				...
				for (Iterator<Element> iter = elem.elementIterator("Host"); iter
						.hasNext();) {
					Element variable = iter.next();
	...
	public void setLatencyModel(GnpLatencyModel model) {
		GnpLatencyModel.init(pingErLookup, countryLookup);
