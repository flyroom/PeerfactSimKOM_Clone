...
public class GnutellaOverlayNodeFactory implements ComponentFactory {
	private static Logger log = SimLogger
			.getLogger(GnutellaOverlayNodeFactory.class);
	...
	public Component createComponent(Host host) {
		return new GnutellaOverlayNode(host.getTransLayer(),
				newGnutellaOverlayID(), this.numConn,
				this.delayAcceptConnection, this.refresh, this.contactTimeout,
				this.descriptorTimeout, port);
	}

	public static GnutellaOverlayID newGnutellaOverlayID() {
		GnutellaOverlayNodeFactory.id += 1;
		log.debug(new GnutellaOverlayID(BigInteger
				.valueOf(GnutellaOverlayNodeFactory.id)));
		return new GnutellaOverlayID(
				BigInteger.valueOf(GnutellaOverlayNodeFactory.id));
	}
