...
import org.peerfact.impl.overlay.dht.chord.carechord.callbacks.JoinOperation;
...
import org.peerfact.impl.overlay.dht.chord.carechord.operations.ChordOperationListener;
import org.peerfact.impl.overlay.dht.chord.carechord.operations.PingOperation;
import org.peerfact.impl.overlay.dht.chord.carechord.operations.SendStabilisationMessagesPeriodicalyOperation;
...
public class ChordNode extends AbstractChordNode {
	...
	public ChordNode(TransLayer transLayer, short port,
		...
		new PingOperation(this)
				.scheduleWithDelay((long) (Math.random() * ChordConfiguration.PING_INTERVAL));
		...
		for (OverlayContact<ChordID> oc : neighbors) {
			...
			if (old != null) {
				createLinkOperation(this, getLocalOverlayContact(), w, old,
						"unmarked");
				// new CreateLinkOperation(this, getLocalChordContact(), old, w,
					...
					if (!(nextSmallest == null && cc.getOverlayID().compareTo(
						...
						while (v.unmarkedEdgeList.contains(cc)) {
							...
							log.debug(v.getChordId() +
									": linLeft: removing from unmarked list: "
									+ cc +
									" nl: " + nl + " ns: " + ns);
						}
					}
				...
				if (!(nextLargest == null && cc.getOverlayID().compareTo(
					...
					if (!(nextLargestReal == null && cc.getOverlayID()
						...
						while (v.unmarkedEdgeList.contains(cc)) {
							...
							String nl = "none";
							if (nextLargestReal != null) {
								nl = nextLargestReal.getOverlayID().toString();
							}
							log.debug(v.getChordId() +
									": linRight: removing from unmarked list: "
									+ cc
									+ " nl: " + nl);
						}
					}
	...
	public void findClosestRealNeighbors() {
		...
		List<AbstractChordContact> realNodeNeighbors = new LinkedList<AbstractChordContact>(
		...
		AbstractChordContact smallestRealNode = null;
		AbstractChordContact largestRealNode = null;
		...
		for (AbstractChordContact n : neighbors) {
			AbstractChordContact prev = null, next = null;
			// find next greater and next smaller realNode:
			for (AbstractChordContact r : realNodeNeighbors) {
				if (r == null) {
					continue;
				}
			...
			if (prev != null) {
				...
				s.add(realNodeNeighbors.get(realNodeNeighbors
						.size() - 1));
			}
			if (next != null) {
				s.add(next);
	...
	public AbstractChordContact getLocalOverlayContact() {
		return new ChordContact(getOverlayID(),
				super.getTransLayer().getLocalTransInfo(this.getPort()), true);
	}
	...
	/**
	 * Starts an update of the k successors
	 */
	public void lookupSuccessors() {
		// update real node sucSuccessor list
		AbstractChordContact senderContact = getLocalOverlayContact();
		AbstractChordContact receiverContact = ((ChordRoutingTable) routingTable)
				.getSuccessor();

		if (receiverContact != null) {
			ChordID virtualNodeIdentifier = getLocalOverlayContact()
					.getOverlayID();
			LookupSuccessorsMessage msg = new LookupSuccessorsMessage(
					senderContact, virtualNodeIdentifier, receiverContact,
					CaReChordConfiguration.SUCCESSORS,
					CaReChordConfiguration.SUCCESSORS);
		...
		// update virtual nodes sucSuccessor list
		for (ChordRoutingTable v : virtualNodes) {
			senderContact = getLocalOverlayContact();
			receiverContact = v.getSuccessor();

			if (receiverContact != null) {
				ChordID virtualNodeIdentifier = v.getChordId();
				LookupSuccessorsMessage msg = new LookupSuccessorsMessage(
						senderContact, virtualNodeIdentifier, receiverContact,
						CaReChordConfiguration.SUCCESSORS,
						CaReChordConfiguration.SUCCESSORS);
