...
import org.peerfact.impl.overlay.dht.chord.epichord.callbacks.CheckPredecessorOperation;
import org.peerfact.impl.overlay.dht.chord.epichord.callbacks.CheckSuccessorOperation;
import org.peerfact.impl.overlay.dht.chord.epichord.callbacks.UpdateDirectSuccessorOperation;
import org.peerfact.impl.overlay.dht.chord.epichord.operations.UpdateChordCacheOperation;
...
public class ChordRoutingTable extends AbstractChordRoutingTable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7334220344054271378L;
	...
	public ChordRoutingTable(ChordNode masterNode,
			AbstractChordContact predecessor, AbstractChordContact successor,
			Set<AbstractChordContact> succFingerTable,
			ChordCache succCache) {
		...
		// EpiChord: create initial cache from successor and add
		// successor/predecessor
		chordCache = new ChordCache(masterNode, succCache);
		chordCache.update(successor);
		chordCache.update(predecessor);

		// EpiChord: schedule periodical cache update
		new UpdateChordCacheOperation(masterNode).scheduleImmediately();
	...
	// * @param offlineNode
	// */
	// public abstract void receiveOfflineEvent(AbstractChordContact
	// offlineNode);
	 ...
	 */
	private transient AbstractChordContact nextUpdatePredecessor,
			nextUpdateSuccessor;
	...
	public void updateDistantSuccessor(AbstractChordContact successor,
		...
		if (succOfSuccessor == null) {
			...
			if (successorList.size() > num_Neighbours) {
				successorList = new LinkedList<AbstractChordContact>(
						successorList.subList(0, num_Neighbours));
			}
	...
	public void updateDistantPredecessor(AbstractChordContact predecessor,
		...
		if (predOfPredecessor == null) {
			...
			if (predecessorList.size() > num_Neighbours) {
				predecessorList = new LinkedList<AbstractChordContact>(
						predecessorList.subList(0, num_Neighbours));
			}
