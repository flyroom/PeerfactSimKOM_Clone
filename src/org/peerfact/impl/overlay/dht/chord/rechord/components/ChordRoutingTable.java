 ...
 * 
 * @author Minh Hoang Nguyen, rewritten for usage in re-chord by Philip Wette,
 *         2011
 * 
 */
public class ChordRoutingTable extends AbstractChordRoutingTable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3532737612218224988L;
	...
	public Collection<AbstractChordContact> getRealNodeNeighbors() {
		...
		for (OverlayContact<ChordID> c : this.getNeighbors()) {
			AbstractChordContact cc = (AbstractChordContact) c;
			if (((ChordContact) cc).isRealNode() && !contacts.contains(cc)) {
				contacts.add(cc);
			}
		}
	...
	public AbstractChordContact getFingerEntry(int index) {
		...
		while (vIt.hasNext()) {
			...
			if (rt.id.getDistance(masterNode.getOverlayID()).equals(
					BigInteger.valueOf(2).pow(index))) {
