...
package org.peerfact.impl.overlay.dht.chord.chord.operations;
...
public class UpdateFingerPointOperation extends
	...
	protected AbstractChordNode masterNode;

	protected int updateIndex;

	protected AbstractChordRoutingTable routingTable;
	...
	public UpdateFingerPointOperation(AbstractChordNode component) {
		super(component);
	...
	protected void execute() {
		...
		if (masterNode.isPresent()) {
			...
			updateIndex = ((ChordRoutingTable) routingTable)
					.getNextUpdateFingerPoint();
	...
	protected class UpdateFingerPointCallback implements
		...
		public void calledOperationSucceeded(
				Operation<List<AbstractChordContact>> op) {
			((ChordRoutingTable) routingTable).incNextUpdateFingerPoint();
			List<AbstractChordContact> responders = op.getResult();
			if (!responders.isEmpty()) {
				responder = responders.get(0);
				((ChordRoutingTable) routingTable).setFingerPoint(updateIndex,
						responder);
