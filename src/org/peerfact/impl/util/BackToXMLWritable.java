...
/**
 * Understanding of the XML configurator of PeerfactSim.KOM may be required to
 * understand this interface.
 * 
 * An XML-configurable class in PeerfactSim.KOM that allows for writing back an
 * already configured object to an output stream or a string. The target is to
 * dump default configurations to the simulator user for a better understanding
 * of the components.
 * 
 ...
 * </pre>
 * 
 * where writer is the java.io.Writer to write to, rootElemName is the name of
 * the root element of the XML document, and obj is the configurable object that
 * shall be written back.
 * 
 * You do not need to use a Writer, you also can get back a string containing
 * your element. For that, use
 * 
 * <pre>
...
public interface BackToXMLWritable {
	...
	/**
	 * Every class that implements this interface usually contains multiple
	 * configurable values of these three types:
	 * <ul>
	 * <li>simulation time units (long),
	 * <li>a primitive type (String, int, long when not used as simulation time
	 * units)
	 * <li>another complex configurable object that implements
	 * BackToXMLWritable.
	 * </ul>
	 * A class implementing BackToXMLWritable has to implement the method
	 * writeBackToXML(BackWriter bw). For every value inside this class of these
	 * three types, writeBackToXML(...) has to call either:
	 * <ul>
	 ...
	 * </ul>
	 * where "name" is the name of the corresponding XML attribute (time, simple
	 * type) or the element (complex type). Maybe this class does not contain
	 * configurable values. Then this method may be empty.
	 * 
	 * @param bw
	 */
	...
	public static class BackWriter {
		 ...
		 * 
		 * @param rootElemName
		 *            : the name of the root element, containing all other
		 *            elements.
		 * @param obj
		 *            : the object that shall be written back
		 * @return
		 */
		public static String getWrittenBackDoc(String rootElemName,
				BackToXMLWritable obj) {
		 ...
		 * 
		 * @param writer
		 *            : the writer to which shall be written back.
		 * @param rootElemName
		 *            : the name of the root element, containing all other
		 *            elements.
		 * @param obj
		 ...
		 * @throws IOException
		 */
		public static void writeBack(Writer writer, String rootElemName,
				BackToXMLWritable obj) throws IOException {
